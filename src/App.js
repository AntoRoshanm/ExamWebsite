...
function AdminPage() {
  ...
  const handleAddQuestionSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await setDoc(doc(db, "questions", currentQuestionId), newQuestion);
        setIsEditing(false);
  ...
  const handleEditQuestion = (question) => {
    ...
    setShowAddQuestionSection(true); // Show the question-adding section when editing
  };
  ...
  const handleShowAddQuestionSection = () => {
  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className="admin-content">
        {!showAddQuestionSection && !showQuestions && (
          <div className="admin-card">
            <h3>What would you like to do?</h3>
            <button onClick={handleShowAddQuestionSection} className="spacebutton">
              <FontAwesomeIcon icon={faPlus} /> Show Question Adding Section
            </button>
            <button onClick={handleShowQuestions} className="spacebutton">
              <FontAwesomeIcon icon={faEye} /> Show Added Questions
            </button>
            <button onClick={handleDownloadExcel} className="spacebutton1">
              <FontAwesomeIcon icon={faDownload} /> Download User Answers
            </button>
          </div>
        )}
        {showAddQuestionSection && (
          <div className="admin-card">
            <h3>{isEditing ? "Edit Question" : "Add a New Question"}</h3>
            ...
            <div className="form-group">
              ...
              <button onClick={handleAddQuestionsFromFile}>
                <FontAwesomeIcon icon={faFileUpload} /> Upload Questions from File
              </button>
            </div>
            <button onClick={() => setShowAddQuestionSection(false)}>
              <FontAwesomeIcon icon={faEyeSlash} /> Hide Question Adding Section
            </button>
          </div>
        )}
        {showQuestions && (
          <div className="admin-card">
            ...
            <button onClick={() => setShowQuestions(false)}>
              <FontAwesomeIcon icon={faEyeSlash} /> Hide Added Questions
            </button>
...
function App() {
  ...
  useEffect(() => {
    fetchQuestions();
    document.addEventListener("click", () => setUserInteracted(true)); // Set interaction on any click
    return () => {
      document.removeEventListener("click", () => setUserInteracted(true));
    };
  }, []);
  ...
  useEffect(() => {
    let sound;
    if (!isAdmin && timeLeft <= 10 && timeLeft >= 1 && userInteracted) {
  ...
  }, [timeLeft, userInteracted, isAdmin]);
